...
As such a policy was developed through the use of Monte Carlo learning as highlighted in the figure below. It is interesting to note that the policy contains more noise than that of Q-learning considered earlier. This could be due to the fact that Monte Carlo methods update Q-values based on complete episodes, which can be subject to high variance, especially when the episodes are long. Variance in episode returns can lead to noisy Q-value updates. Furthermore, this could be explained by the fact that in Q-learning, you're bootstrapping by updating the Q-value of the current state-action pair based on the estimated Q-value of the next state. This can introduce more stability into the learning process compared to Monte Carlo methods that rely on returns. In Monte Carlo methods, the importance sampling ratio (defined in the pseudo code above as W) can introduce additional variance. These ratios are used to correct for the difference between the behaviour policy (used during exploration) and the target policy (policy being learned).
